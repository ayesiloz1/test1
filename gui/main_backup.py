...
class WeldDefectGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.processing_delay = 0  # Delay between batch processing (0-5 seconds)
        self.is_paused = False  # Pause state for batch processing
        self.processing_delay = 0  # Delay between batch processing (0-5 seconds)
    ...
    def create_left_panel(self):
        ...
        self.slider_speed.setMaximum(50)  # 0 to 5.0 seconds (in 0.1s increments)
    ...
    def on_speed_changed(self, value):
        ...
        self.processing_delay = value / 10.0  # Convert to seconds (0.0 to 5.0)
        self.lbl_speed_value.setText(f"{self.processing_delay:.1f}s")
    ...
    def toggle_pause(self):
        ...
        if self.is_paused:
            ...
            self.btn_pause.setStyleSheet("font-size: 12px; background-color: #00aa00;")
    ...
    def update_defect_detection_view(self, result):
        ...
        try:
            ...
            # Calculate bounding box from AE reconstruction difference (not heatmap)
            bounding_boxes = []
            if result['autoencoder'] and result['autoencoder'].get('reconstruction') is not None:
                ...
                # Get reconstruction
                reconstruction = result['autoencoder']['reconstruction'].unsqueeze(0).cpu()
                
                # Calculate absolute difference between original and reconstruction
                diff = torch.abs(original_normalized - reconstruction)
                diff = torch.mean(diff, dim=1).squeeze().numpy()  # Average across channels
                ...
                # Use higher threshold to focus on actual defects, not noise
                ...
                # Remove edge artifacts (ignore outer 10 pixels)
                binary_mask[0:10, :] = 0
                binary_mask[-10:, :] = 0
                binary_mask[:, 0:10] = 0
                binary_mask[:, -10:] = 0
                ...
                if contours:
                    # Get significant contours (larger than 50 pixels to filter noise)
